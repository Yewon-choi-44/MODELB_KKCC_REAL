...
산업 현장에서 AI를 활용한 불량 검출 시 가장 큰 걸림돌은 정상 제품에 비해 불량품 데이터가 극도로 부족한 '데이터 희소성 및 불균형' 문제입니다. <br>
**model B**는 학습 효율을 완벽하게 서포트하기 위해, 
* 생성형 AI인 **Stable Diffusion 3.5(SD3.5)**를 기반으로 형태를 정밀 제어하는 **ControlNet**
* 소량 데이터 맞춤형 고효율 미세조정 기술인 **DoRA**
* 그리고 미세 질감을 현미경 수준으로 다듬는 **SUPIR** 기술을 결합한 **조립식 모듈형** 파이프라인을 구축했습니다. 
...
### 요약 : Stable Diffusion 3.5 Large + Full fine tunning (DoRA 대체) + ControlNet (Canny + Depth) + SUPIR

* **Stable Diffusion 3.5 Large** : MMDiT(Multimodal Diffusion Transformer) 구조를 활용한 이미지 생성

* **Full fine tunning** : Stable Diffusion 3.5 Large 파라미터 전체 학습하여 이미지 생성 시 적용함
  <br> → Full fine tunning 적용이 어려울 것이 우려되어 **DoRA 가중치**를 사용.
  <br> → Full fine tunnung 적용 가능 여부가 확인되면 해당 과정 추가 예정.

* **ControlNet (Canny + Depth)** : Canny Edge를 추출하여 경계선, 윤곽선 등의 정확한 위치를 지정하고, Depth Map을 통해 이미지 평면화 및 왜곡을 방지함.

* **SUPIR** : 원본의 미세한 스크래치, 균열, 질감까지 고해상도로 복원함
...
* MVTec AD 2 불량 데이터 증강 파이프라인 (Defect Data Augmentation Pipeline)
